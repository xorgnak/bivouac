...
source ~/.prompt
function commit() {
git add . && git commit -m "$(gum input --value "$(gum choose "progress" "bug" "milestone" "pretty" "docs" "debug") -> " --placeholder "summary...")" -m "$(gum input --placeholder "details..")" && git push && git log -10 --pretty=format:'%Cred%h%Creset -%C(yellow)%d%Creset %s %Cgreen(%cr) %C(bold blue)<%an>%Creset' --abbrev-commit
}
